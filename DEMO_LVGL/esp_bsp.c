...
esp_err_t bsp_display_new(const bsp_display_config_t *config,
  ...
  const axs15231b_vendor_config_t vendor_config = {
      .init_cmds = lcd_init_cmds, // Uncomment these line if use custom
...
static lv_display_t *bsp_display_lcd_init(const bsp_display_cfg_t *cfg) {
  ...
  /* In LVGL 9 with native rotation, we pass physical resolution here. */
  /* LVGL will swap them internally if rotation is 90/270. */
  disp_cfg.hres = hres;
  disp_cfg.vres = vres;
